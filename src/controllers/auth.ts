...
export const registerUser = TryCatch(async (req, res, next) => {
    ...
    if (role === "recruiter") {
        ...
        // Resume is uploaded by REST API instead of kafka because of the need of response from utils service after upload. 
    ...
    res.status(200).json({ message: "User registered", registeredUser, token });
});
...
export const loginUser = TryCatch(async (req, res, next) => {
    ...
    const token = jwt.sign({ id: userObject?.user_id }, process.env.JWT_SECRET as string, { expiresIn: "15d" });
...
export const forgotPassword = TryCatch(async (req, res, next) => {
    ...
    const resetToken = jwt.sign({ email: user.email, type: "reset" }, process.env.JWT_SECRET as string, { expiresIn: '15m' });
    ...
    const message = {
        to: email,
        subject: "RESET your password - CareerNest",
